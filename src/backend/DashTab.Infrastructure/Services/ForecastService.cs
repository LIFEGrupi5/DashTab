    ...
    ILogger<ForecastService> logger) : IForecastService
{
    ...
    public async Task<RevenueForecastResponse> GetRevenueForecastAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var resp = await client.PostAsJsonAsync($"{baseUrl}/forecast", payload, ct);
            ...
            var result = await resp.Content.ReadFromJsonAsync<RevenueForecastResponse>(JsonOpts, ct);
