...
function MarketingNav() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 border-b border-white/10 bg-stone-950/70 backdrop-blur-md">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <Link href="/dashboard"
              className="px-4 py-2 rounded-lg bg-orange-500 hover:bg-orange-400 text-white text-sm font-semibold transition shadow shadow-orange-500/30">
              Go to Dashboard
            </Link>
          ) : (
            <>
        ...
        {/* Mobile toggle */}
        <button onClick={() => setMobileOpen(o => !o)} className="md:hidden p-2 text-stone-300">
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden border-t border-white/10 bg-stone-950 px-6 py-4 flex flex-col gap-4">
          {NAV_LINKS.map(l => (
          ...
          <div className="pt-2 border-t border-white/10 flex flex-col gap-2">
            {user ? (
              ...
              <>
                <Link href="/login" className="text-sm text-stone-400 text-center">Log in</Link>
  ...
  return (
    <footer className="border-t border-white/10 bg-stone-950 text-stone-400">
      <div className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 sm:grid-cols-3 gap-8">
        ...
        <div>
          <p className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-3">Product</p>
          <ul className="space-y-2 text-sm">
            {NAV_LINKS.map(l => <li key={l.href}><Link href={l.href} className="hover:text-orange-400 transition">{l.label}</Link></li>)}
          </ul>
        </div>
        <div>
          <p className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-3">Get started</p>
          <ul className="space-y-2 text-sm">
            <li><Link href="/register" className="hover:text-orange-400 transition">Create account</Link></li>
      ...
      <div className="border-t border-white/10 px-6 py-4 text-center text-xs text-stone-500">
